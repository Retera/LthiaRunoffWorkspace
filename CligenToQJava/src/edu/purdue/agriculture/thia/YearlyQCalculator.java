...
public class YearlyQCalculator {
	...
	private final void sAndpMinCalculation() {
		...
		for (itt = 0; itt < cnArray.length; itt++) {
			...
			sArray[itt] = LthiaUtils.computeS(curveNumber, LthiaUtils.Units.MILLIMETERS);
	...
	public void convertToQRunoff(final String inFile, final Workspace workspace) throws IOException {
		...
		/* RandomAccessFile class is used so that seek option is available */
		try (RandomAccessFile inputRainfallFile = new RandomAccessFile(workspace.getInputFile(inFile + ".txt"), "r")) {
			...
			try {
				/* VARIABLES */
				int count = 1; /*
								 * To count the file lines so that we can skip
								 * them accordingly
								 */
				...
				/*
				 * Getting data from the 3rd, 5th and 14th line in the input
				 * file
				 */
				...
				/*
				 * At this point we have : simulated yearlength, obs years
				 * version number, station name, col place of precp and year
				 */
				 ...
				 * being taken from the input file and printed in the output
				 * file
				 */

				/*
				 * Temporary array to stores the sum-of-daily-Q generated for
				 * one particular year
				 */
				...
				while (((line = inputRainfallFile.readLine()) != null)) {
					...
					/*
					 * Getting year printed with the precipitation data in the
					 * input file
					 */
					...
					/*
					 * Initially check is 2 and then it changes with the
					...
					if (year == check) {
						 ...
						 *
						 *
						 * //Print P and make it null----- to check runoff less
						 * that precipitation
						 *
					...
					/*
					 * In getYear() : when the last line of the file is found,
					 * which is an empty line, the function returns -1. Since we
					 * print the qArray after a particular line is processed, we
					 * need to check for this empty line to print the last
					 * processed qArray.
					 */
					if (year == -1) {
						 ...
						 * RUNOFFcheck :
						 *
						 * // Print P and make it null so as to check if runoff
						 * is less that precipitation
						 *
						 * System.out.print("year:"+(year-1)+"\t\t") ;
						 *
						 * for(i = 0 ; i < qArray.length; i++) {
					...
					for (i = 0; i < qArray.length; i++) {
						final double qValue = LthiaUtils.computeQ(precp, pMinArray[i], sArray[i]);
						qArray[i] += qValue;
					} // for ends here
